...
\title{\textbf{PlatePlanner: An AI-Powered Meal Planning System with\\
Graph-Based Ingredient Substitution, Food Ontology Reasoning,\\
and Ethnicity-Aware Recipe Recommendation}}

\author{
\IEEEauthorblockN{Sandilya Chimalamarri}
\IEEEauthorblockA{San Jos\'e State University\\
ML Research Lead\\
\texttt{sandilya.chimalamarri@sjsu.edu}}
\and
\IEEEauthorblockN{Sai Priyanka Bonkuri}
\IEEEauthorblockA{San Jos\'e State University\\
Graph Database Architect\\
\texttt{saipriyanka.bonkuri@sjsu.edu}}
\and
\IEEEauthorblockN{Pavan Charith Devarapalli}
\IEEEauthorblockA{San Jos\'e State University\\
Backend Systems Engineer\\
\texttt{pavancharith.devarapalli@sjsu.edu}}
\and
\IEEEauthorblockN{Sai Dheeraj Gollu}
\IEEEauthorblockA{San Jos\'e State University\\
Data Pipeline Engineer\\
\texttt{saidheeraj.gollu@sjsu.edu}}
}
...
\begin{abstract}
PlatePlanner is an AI-powered meal planning and recipe recommendation platform that integrates
multiple machine learning models to deliver personalized, culturally-aware, and nutritionally
sound meal plans. The system combines (1) a FAISS-based semantic retrieval engine using
fine-tuned sentence transformers, (2) a Graph Neural Network (GraphSAGE) trained on a Neo4j
ingredient knowledge graph for context-aware ingredient substitution, (3) a food ontology layer
encoding culinary rules and dietary constraints, and (4) a Retrieval-Augmented Generation (RAG)
pipeline leveraging large language models for recipe adaptation. Evaluated on the RecipeNLG
dataset (2.2M+ recipes) augmented with international cuisine corpora, PlatePlanner achieves a
GNN substitution AUC of 0.87, semantic retrieval latency under 100ms at p95, 92\% user
satisfaction on context-aware substitutions, and dietary classification accuracy exceeding 90\%
across 12 diet types and 11 allergen categories.
\end{abstract}

\begin{IEEEkeywords}
Recipe Recommendation, Graph Neural Networks, Knowledge Graphs, Ingredient Substitution,
Retrieval-Augmented Generation, Dietary Classification, Food Ontology, Meal Planning.
\end{IEEEkeywords}
